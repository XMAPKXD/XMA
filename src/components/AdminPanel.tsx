...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [adminUnlockTab, setAdminUnlockTab] = useState<'direct' | 'google'>('direct');
  const [directEmailInput, setDirectEmailInput] = useState<string>('kawanyuri35@gmail.com');
  const [directPinInput, setDirectPinInput] = useState<string>('XMA2026');
  ...
  const handleGoogleAdminSignIn = async () => {
    try {
      ...
      setGoogleError('A janela de autenticação do Google foi restrita pelo navegador. Use o Acesso Direto com PIN Master ou clique no botão de acesso do organizador Kawan Yuri.');
      setAdminUnlockTab('direct');
    } finally {
      setIsGoogleLoading(false);
    }
  };

  const handleDirectAdminUnlockWithPin = (customEmail?: string, customPin?: string) => {
    setGoogleError(null);
    const emailToVerify = (customEmail || directEmailInput || '').trim().toLowerCase();
    const pinToVerify = (customPin !== undefined ? customPin : directPinInput || '').trim();

    if (!isAuthorizedAdminEmail(emailToVerify)) {
      setGoogleError(`O e-mail "${emailToVerify}" não possui privilégios de Administrador Oficial.`);
      return;
    }

    if (pinToVerify && !isValidAdminPin(pinToVerify)) {
      setGoogleError('PIN incorreto. O PIN mestre padrão é: XMA2026');
      return;
    }
    ...
    if (onLoginAdmin) {
      onLoginAdmin(adminName, adminTag, adminAvatar, emailToVerify);
    }
  ...
  const handleLock = () => {
    ...
    try {
      ...
      localStorage.removeItem('xma_admin_session_unlocked');
    } catch {}
  };
    ...
    return (
      <div className="max-w-md mx-auto py-12 px-4 space-y-6">
        <div className="rounded-3xl bg-[#14151e] border-2 border-amber-500/50 p-6 sm:p-8 shadow-2xl space-y-6 text-center">
          ...
          {userAccount?.isLoggedIn && isUserEmailAdmin ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex rounded-xl bg-zinc-900/90 p-1 border border-zinc-800">
                <button
                  type="button"
                  onClick={() => setAdminUnlockTab('direct')}
                ...
                <button
                  type="button"
                  onClick={() => setAdminUnlockTab('google')}
                  className={`flex-1 py-2 px-3 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                    adminUnlockTab === 'google'
                      ? 'bg-amber-400 text-black shadow-md font-black'
                      : 'text-zinc-400 hover:text-white'
                  }`}
                >
              ...
              {adminUnlockTab === 'direct' && (
                <div className="space-y-3.5">
                  {/* Quick 1-Click Access for Kawan Yuri */}
                  <div className="p-3.5 rounded-2xl bg-gradient-to-r from-amber-950/60 via-amber-900/40 to-black/80 border border-amber-500/50 space-y-2.5 text-left">
                    ...
                    <button
                      type="button"
                      onClick={() => handleDirectAdminUnlockWithPin('kawanyuri35@gmail.com', 'XMA2026')}
                  ...
                  <form
                    onSubmit={(e) => {
                      e.preventDefault();
                      handleDirectAdminUnlockWithPin();
                    }}
                    className="space-y-3 text-left pt-1 border-t border-zinc-800"
                  >
                    ...
                    >
                      ...
                      <span>Validar PIN e Liberar Painel</span>
                    </button>
                  </form>
                </div>
              )}
              ...
              {adminUnlockTab === 'google' && (
                <div className="space-y-3 pt-1">
                  ...
                  <p className="text-[11px] text-zinc-500">
                    Contas autorizadas: kawanyuri35@gmail.com, eukoosh@gmail.com
                  </p>
                </div>
              )}
            </div>
          ...
          {googleError && (
            <div className="p-3 rounded-xl bg-red-900/30 border border-red-500/50 text-red-300 text-xs text-left space-y-1">
